...
export default function DatabasePage({ user }: DatabasePageProps) {
  ...
  const isTest = sid !== 'new' && conns.find(c => c.id === sid)?.isTest === 1;
  return (
    <div className="min-h-full">
      <div className="max-w-7xl mx-auto p-6">
        <header className="mb-6">
          <h1 className="text-sm font-semibold text-slate-900">Database Connections</h1>
          <p className="text-xs text-slate-500 mt-0.5">manage connection endpoints</p>
        </header>
        <div className="grid grid-cols-12 gap-6 items-start">
          <section className="col-span-12 lg:col-span-4 space-y-3">
            <div className="bg-white border border-slate-200 rounded-lg overflow-hidden">
              <div className="p-3 border-b border-slate-200 flex justify-between items-center">
                <h3 className="text-xs font-semibold text-slate-900">Connections</h3>
                <span className="text-[10px] text-slate-500">{conns.length} active</span>
              </div>
              <div className="max-h-[400px] overflow-y-auto divide-y divide-slate-100">
                {loading ? <div className="p-6 text-center text-xs text-slate-500">Loading...</div>
                : conns.length === 0 ? <div className="p-6 text-center text-xs text-slate-500">No connections</div>
                : conns.map((c: any) => (
                  <div key={c.id} onClick={() => setSid(c.id)}
                    className={'p-3 hover:bg-slate-50 cursor-pointer transition-colors ' + (sid === c.id ? 'bg-blue-50 border-l-2 border-blue-600' : '')}>
                    <div className="text-sm text-slate-900">{c.name}{c.isTest === 1 ? ' (test)' : ''}</div>
                    <div className="text-[10px] text-slate-500 mt-0.5">{c.host} / {c.dbType}</div>
                  </div>
                ))}
              </div>
              <button onClick={() => setSid('new')}
                className="flex items-center justify-center gap-2 w-full py-2.5 text-xs text-blue-600 hover:bg-blue-50 border-t border-slate-200 transition-colors">
          ...
          <section className="col-span-12 lg:col-span-8">
            {sid === null ? (
              <div className="bg-white border border-slate-200 rounded-lg p-10 flex flex-col items-center justify-center text-slate-400 text-xs">
              ...
              <div className="bg-white border border-slate-200 rounded-lg p-6">
                <div className="flex items-center justify-between mb-5 pb-4 border-b border-slate-200">
                  ...
                  <select className="bg-slate-50 border border-slate-200 text-xs text-slate-900 px-2 py-1 rounded outline-none focus:border-blue-500"
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="md:col-span-2">
                      <label className="block text-[10px] uppercase tracking-wider text-slate-500 mb-1">Connection Name</label>
                      <input type="text" placeholder="e.g. Production Data Warehouse"
                        className="w-full bg-slate-50 border border-slate-200 text-slate-900 text-xs px-3 py-2 rounded outline-none focus:border-blue-500 disabled:opacity-50"
                        value={f.name} onChange={(e) => setF({ ...f, name: e.target.value })} disabled={isTest} />
                    </div>
                    <div>
                      <label className="block text-[10px] uppercase tracking-wider text-slate-500 mb-1">Host</label>
                      <input type="text" placeholder="localhost or 192.168.1.100"
                        className="w-full bg-slate-50 border border-slate-200 text-slate-900 text-xs px-3 py-2 rounded outline-none focus:border-blue-500 disabled:opacity-50"
                        value={f.host} onChange={(e) => setF({ ...f, host: e.target.value })} disabled={isTest} />
                    </div>
                    <div>
                      <label className="block text-[10px] uppercase tracking-wider text-slate-500 mb-1">Port</label>
                      <input type="number"
                        className="w-full bg-slate-50 border border-slate-200 text-slate-900 text-xs px-3 py-2 rounded outline-none focus:border-blue-500 disabled:opacity-50"
                        value={f.port} onChange={(e) => setF({ ...f, port: parseInt(e.target.value) || 0 })} disabled={isTest} />
                    </div>
                    <div>
                      <label className="block text-[10px] uppercase tracking-wider text-slate-500 mb-1">Username</label>
                      <input type="text"
                        className="w-full bg-slate-50 border border-slate-200 text-slate-900 text-xs px-3 py-2 rounded outline-none focus:border-blue-500 disabled:opacity-50"
                  ...
                  <div className="flex items-center justify-between pt-5 border-t border-slate-200">
                    {sid !== 'new' && !isTest ? (
                      <button onClick={() => setConfirmDelete(true)}
                        className="px-4 py-2 border border-red-300 text-red-600 text-xs hover:bg-red-50 rounded transition-colors flex items-center gap-1.5">
                    ...
                    <div className="flex items-center gap-3">
                      <button onClick={testConn} disabled={testing}
                        className="flex items-center gap-1.5 px-4 py-2 border border-blue-300 text-blue-600 text-xs hover:bg-blue-50 rounded transition-colors disabled:opacity-50">
                        <RefreshCw size={13} className={testing ? 'animate-spin' : ''} /> {testing ? 'Testing...' : 'Test'}
                      </button>
                      {!isTest && (
                        <button onClick={saveConn} disabled={saving}
                          className="flex items-center gap-1.5 px-5 py-2 border border-blue-300 bg-blue-50 text-blue-600 text-xs hover:bg-blue-100 rounded transition-colors disabled:opacity-50">
                          {saving ? 'Saving...' : 'Save'}
                        </button>
                      )}
                    </div>
        ...
        {toast && (
          <div className={'fixed bottom-6 right-6 z-50 px-4 py-3 border rounded text-xs flex items-center gap-2 shadow-lg ' + (toast.type === 'ok' ? 'border-emerald-300 text-emerald-600 bg-emerald-50' : 'border-red-300 text-red-600 bg-red-50')}>
            <span>{toast.msg}</span>
